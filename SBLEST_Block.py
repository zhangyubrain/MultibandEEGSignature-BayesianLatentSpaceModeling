...
#%%
def SBLEST2(X,X1,X2,Y, K, tau, components, Epoch=5000, epoch_print=100):
    """
    ...
    X1         : M trials of C (channel) x T (time) EEG signals. [C, T, M].
    X2         : M trials of C (channel) x T (time) EEG signals. [C, T, M].
    ...
    R_train, Wh = Enhanced_cov(X,X1,X2, K, tau, components, train=1)
    # print('\n')
...
def create_cov(X,K,tau):
    X_order_k = None
    C, T, M = X.shape
    Cov = zeros(M,K*C,K*C).to(device)
    for m in range(M):
        X_m = X[:, :, m]
...
def block_cov(X,X1,X2):
    M, C,C = X.shape
    # Assuming three oscillations
    Block = zeros(M,C*3,C*3, dtype=float64).to(device)
    for i in range(M):
        Block[i] = torch.block_diag(X[i], X1[i], X2[i]) 
    
    return Block

def Enhanced_cov(X,X1,X2, K, tau, Wh=None, train=1):
    """
    ...
    --- Returns ---
    R         : Enhanced block covariance matrix. [M,(K*C)^2*(K*C)^2 ]
    Wh : Whitening matrix. [(K*C)^2, (K*C)^2].
    """

    # Initialization
    #Calculate covariance of each oscillation
    X= create_cov(X, K, tau)
    X1= create_cov(X1, K, tau)
    X2= create_cov(X2, K, tau)
    #Create a block diagonal matrix
    Cov= block_cov(X, X1, X2)
    M, C,_ = Cov.shape
    
    
    
    # Sum along the first axis
    Sig_Cov = torch.sum(Cov, dim=0)
